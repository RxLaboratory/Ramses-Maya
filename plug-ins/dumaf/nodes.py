...
def renameShapes( node ):
    ...
    if shapes is None: return
    for shape in shapes:
        cmds.rename(shape, paths.baseName(node) + 'Shape')  
...
def parent( child, parent, r=False):
    ...
    child = paths.baseName( child, True)
...
def snap( nodeFrom, nodeTo):
    ...
    nodeFrom = parent( nodeFrom, nodeTo, True )

    if prevParent is not None:
        nodeFrom = parent( nodeFrom, prevParent )
        return nodeFrom
    
    nodeFrom = parent( nodeFrom, '|' )
    return nodeFrom
